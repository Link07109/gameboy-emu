...
static void proc_cb(cpu_context* ctx) {
    ...
    switch (bit) {
        case 0: { // RLC
            ...
            cpu_set_flags(ctx, result == 0, false, false, c);
        } return;
        case 1: { // RRC
            u8 old = reg_val;
            reg_val >>= 1;
...
static void proc_daa(cpu_context* ctx) {
    ...
    cpu_set_flags(ctx, ctx->regs.a == 0, -1, 0, c);
}
...
static void proc_cp(cpu_context* ctx) {
    ...
    int h = (int)(ctx->regs.a & 0x0F) - (int)(ctx->fetched_data & 0x0F);

    cpu_set_flags(ctx, n == 0, 1, h < 0, n < 0);
}
...
static void proc_sub(cpu_context* ctx) {
    u16 read_reg_1 = cpu_read_reg(ctx->cur_inst->reg_1);
    u16 fetched = ctx->fetched_data;

    u16 val = read_reg_1 - fetched;

    int z = val == 0;
    int h = (int)(read_reg_1 & 0x0F) - (int)(fetched & 0x0F) < 0;
    int c = (int)read_reg_1 - (int)fetched < 0;
...
static void proc_sbc(cpu_context* ctx) {
    ...
    u16 val = read_reg_1 - (u8)(fetched + CPU_FLAG_C);

    int z = val == 0;
    int h = (int)(read_reg_1 & 0x0F) - (int)(fetched & 0x0F) - (int)CPU_FLAG_C < 0;
    int c = (int)read_reg_1 - (int)fetched - (int)CPU_FLAG_C < 0;
...
static void proc_add(cpu_context* ctx) {
    ...
    if (is_16bit && ctx->cur_inst->reg_1 != RT_SP) {
        ...
        c = (u32)cpu_read_reg(ctx->cur_inst->reg_1) + (u32)ctx->fetched_data >= 0x10000;
    }
